...
Redis 的跳跃表由 `redis.h/zskiplistNode` 和 `redis.h/zskiplist` 两个结构定义， 其中 `zskiplistNode` 结构用于表示跳跃表节点， 而 `zskiplist`结构则用于保存跳跃表节点的相关信息， 比如节点的数量， 以及指向表头节点和表尾节点的指针， 等等。下面是一个跳跃表的图，

<img src="/images/wiki/Redis/skiplist_redis.png" width="700" alt="跳跃表">
...
<img src="/images/wiki/Redis/redis_intset.png" width="700" alt="跳跃表">
...
<img src="/images/wiki/Redis/ziplist_structure.png" width="700" alt="压缩列表">
...
每个压缩列表节点可以保存一个字节数组或者一个整数值，以下是一个节点的组成：

<img src="/images/wiki/Redis/node_entry.png" width="700" alt="节点">
